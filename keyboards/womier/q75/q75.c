...
void rgb_matrix_wls_indicator(void) {
    ...
    if (wls_rgb_indicator_timer) {
        ...
        if (wls_rgb_indicator_times % 2) {
            ...
            rgb_matrix_set_color(82, wls_rgb_indicator_rgb.r, wls_rgb_indicator_rgb.g, wls_rgb_indicator_rgb.b);
        } else {
...
// 充电接入
    ...
    if (!readPin(BT_CABLE_PIN)) {
        if (!readPin(BT_CHARGE_PIN)) {
            if (timer_elapsed32(charging_time) >= 2000) rgb_matrix_set_color(83, 140,0,0);
        } else {
            charging_time = timer_read32();
            rgb_matrix_set_color(83, 0, 140, 0);
        }
    }
#    endif
